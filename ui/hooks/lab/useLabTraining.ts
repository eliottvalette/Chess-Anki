...
export function useLabTraining(
  ...
  const beginDeckCardSession = useCallback((card: DeckCard, lines: OpeningSeedLine[]) => {
    ...
    playSound('game-start');
    return 0;
  }, [clearSelection, clearVariation, deckCardPromptStartedAtRef, deckReplayInitialFenRef, deckReplayMovesRef, modeRef, persistReviewWorkspaceSnapshot, playSound, setActiveDeckCard, setDeckFeedback, setDeckFeedbackArrowsVisible, setFileName, setGame, setHistoryIndex, setInitialFen, setMetadata, setMode, setMoveHistory, setOrientation, setPositionAnalysis, setPreMoveAnalyses, setShowArrow, setTimelineAnalyses, setTimelineError, timelineRefineRequestIdRef]);
  ...
  const playDeckReplayToIndex = useCallback(async (targetIndex: number, trainSide: DeckCard['side']) => {
    ...
    return false;
  }, [clearSelection, clearVariation, deckReplayInitialFenRef, deckReplayMovesRef, playSoundSequence, setDeckFeedbackArrowsVisible, setDeckPlaybackBusy, setGame, setHistoryIndex]);
  ...
  }, [beginDeckCardSession, deckCardPromptStartedAtRef, playDeckReplayToIndex]);
  ...
  }, [setDeckCardsLoading, setDeckLibraryLoading, setDeckLoadError, setDeckSummaries, setOpeningLines, setDeckCards, setSelectedDeckId, setTrainAllQueue, setTrainSessionIndex, setTrainSessionStats, setDeckIndex, setTrainAllSession, startDeckCardWithReplay]);
