...
export function useLabDeckManager(
  state: LabState,
  context: {
    loadTrainingDeck: (deckId?: string | null, options?: { autoStart?: boolean; allDecks?: boolean; libraryLoading?: boolean }) => Promise<void>;
  }
) {
  const {
    trainingProfile,
  ...
  const createTrainingDeck = useCallback(async (name: string, pgn?: string) => {
  ...
  }, [chesscomUsername, loadTrainingDeck, recentGameTimeClass, selectedDeckId, setDeckActionError, setDeckActionLoading, setSelectedDeckId, trainingProfile?.username, trainingUsername]);

  const renameTrainingDeck = useCallback(async (deckId: string, name: string) => {
    setDeckActionLoading(true);
    setDeckActionError('');
  ...
  const deleteTrainingDeck = useCallback(async (deckId: string) => {
    setDeckActionLoading(true);
    setDeckActionError('');
  ...
  }, [loadTrainingDeck, selectedDeckId, setActiveDeckCard, setDeckActionError, setDeckActionLoading, setDeckCards, setDeckFeedback, setDeckFeedbackArrowsVisible, setDeckProgress, setSelectedDeckId]);
